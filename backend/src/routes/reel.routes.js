...
router.post("/", isAuthenticated, upload.single("video"), validate(createReelSchema), createReel);

router.get("/:reelId", isAuthenticated, getReelById);
router.patch("/:reelId", isAuthenticated, validate(updateReelSchema), updateReel);
...
router.post("/:reelId/comments", isAuthenticated, validate(reelCommentSchema), addReelComment);
